...
void MD5::update(FILE * pFile) {
	if(pFile == NULL)
		return;

#if defined(WIN32) && !defined(__CYGWIN__)
	_fseeki64(pFile,0L,SEEK_END);
	__int64 nSize		= _ftelli64(pFile);
	_fseeki64(pFile,0L,SEEK_SET);
#else
    fseek(pFile,0L,SEEK_END);
	long long nSize = ftell(pFile);
	fseek(pFile,0L,SEEK_SET);
	//fseeko64(pFile,0L,SEEK_END);
	//long long nSize = ftello64(pFile);
	//fseeko64(pFile,0L,SEEK_SET);
#endif

	char buffer[BUFFER_SIZE];
	int length = BUFFER_SIZE;
	for (int i = 0;i < nSize;i += length)
	{
		if(i + BUFFER_SIZE > nSize)
			length = nSize - i;
